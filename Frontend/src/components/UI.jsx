  ...
  return (
    <div className="epms-stat-card epms-fade-up" style={{
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14 }}>
        <div style={{ width:38, height:38, borderRadius:10, background:`${color}12`, border:`1px solid ${color}20`, display:'flex', alignItems:'center', justifyContent:'center', color, transition:'transform 0.2s cubic-bezier(0.34,1.56,0.64,1)' }}
...
export function ProgressBar({ value, max=100, color='indigo', height=6, showLabel=false }) {
  ...
  const colorMap = { indigo:'#4f46e5', green:'#10b981', blue:'#0ea5e9', red:'#f43f5e', amber:'#f59e0b', gray:'#94a3b8', warn:'#f59e0b', purple:'#8b5cf6' };
    ...
    <div ref={barRef}>
      ...
      <div style={{ height, background:'var(--bg-elevated)', borderRadius:height, overflow:'hidden' }}>
        <div style={{
          ...
          borderRadius:height,
          transition:'width 6s cubic-bezier(0.05,0.02,0.15,1)',
...
export function SectionHeader({ title, subtitle, action }) {
  return (
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
      <div>
        <h3 style={{ fontFamily:'var(--font-body)', fontWeight:700, fontSize:18, color:'var(--role-color, #4f46e5)', margin:0 }}>{title}</h3>
...
export function BarChart({ data, color='#6C63FF', height=180 }) {
  ...
  React.useEffect(() => {
    if (animatedRef.current) return; // never reset once played
  ...
  const COLORS = [
    '#6C63FF','#43E8AC','#FFB547','#f43f5e','#38BDF8','#A78BFA','#fb923c','#FF6584'
  ];

  if (!data || data.length === 0) return null;

  const values = data.map(d => d.score ?? d.rate ?? d.value ?? 0);
  const max = 100; // fixed max so all bars share the same baseline scale
        ...
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: chartH, paddingBottom: 44, paddingTop: 32, position: 'relative', zIndex: 1 }}>
            {data.map((d, i) => {
              ...
              const ABBR = { Marketing:'Mktg', Finance:'Fin', Operations:'Ops', Engineering:'Eng', Administration:'Admin' };
              const shortLabel = ABBR[label] || (label.length > 6 ? label.slice(0, 5) + '…' : label);
              ...
              return (
                ...
                >
                  ...
                  <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                    {/* Value label — absolutely above bar, never affects layout */}
                    <div style={{
                      position: 'absolute', bottom: `${pct * barAreaH + 6}px`,
                    ...
                    <div style={{
                      ...
                      transform: animated
                        ...
                        : 'scaleY(0) scaleX(1)',
                      transition: `transform 0.85s cubic-bezier(0.22,1,0.36,1) ${i * 0.08}s, box-shadow 0.18s ease, background 0.18s ease`,
                      position: 'relative', overflow: 'hidden',
                    }}>
                      {/* Shine */}
                      <div style={{
                  ...
                  {/* X label */}
                  <div style={{
                    marginTop: 8, fontSize: 12, fontWeight: 700, color: isHov ? barColor : 'var(--text-secondary)',
