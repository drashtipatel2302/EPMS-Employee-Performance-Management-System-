...
export const sendJoiningLetter = async (id, tempPassword = '') => {
  const res = await fetch(`/api/auth/employees/${id}/send-joining-letter`, {
    method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ tempPassword }),
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || 'Failed to send joining letter');
  return data;
};
